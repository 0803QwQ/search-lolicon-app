...
if (!$asked){
    ...
    foreach ($array as $setuKey => $setuArray){
        ...
        foreach ($setuArray['tags'] as $key => $value){
            ...
            if ($key < ($setShowTags - 1)){
                ...
                if ($setShowTags != count($setuArray['tags'])) $setuTagEnd = "...";
    ...
    printSetu:
    $htmlDoc = htmlHeader("keyword")."<input id='license' type='license' name='license' value='{$setLicense}' style='display:none'><input id='keyword' type='keyword' name='keyword' value='{$keyword}' placeholder='请输入搜索关键词'><br>{$chooseR18}获取<span id='numberInfo'>{$numberInfo}</span><input id='submit' type='submit' value='立即搜索',name='submit'></form>{$htmlSetu}</h3></div></div></div><script>function download(info){alert('正在唤起浏览器下载...');location.href='?download='+info;};function radioCheck1(){document.getElementById('numberInfo').innerHTML=\"数量(1-{$maxNumber})：<input id='number' type='number' name='number' value='{$number}' min='1' max='{$maxNumber}' />张\";};function radioCheck2(){document.getElementById('numberInfo').innerHTML=\"页码：<input id='number' type='number' name='number' value='{$number}' min='1' max='2147483647' />页\";};</script></body><html>";
};
print_r($htmlDoc);
exit;
?>
