...
function htmlHeader($page){
    global $keyword, $over18, $setBackground, $setBgBlur;
...
if (!$asked){
    ...
    if ($license == $setLicense){
        ...
        if ($array == array()){
            ...
            if ($over18 == 1) $chooseR18 = "<br>全年龄<input type='radio' {$r18Check0}name='r18' value='0'>&emsp;&emsp;R18<input type='radio' {$r18Check1}name='r18' value='1'>&emsp;&emsp;随机<input type='radio' {$r18Check2}name='r18' value='2'>";
        };
        $return = htmlHeader("keyword")."<input type='license' name='license' value='{$setLicense}' style='display:none'><input type='keyword' name='keyword' value='{$keyword}' placeholder='请输入搜索关键词'>{$chooseR18}<br>获取图片数量(1-{$setMaxNumber})：<input type='number' name='number' value='{$number}' min='1' max='{$setMaxNumber}' />张<input type='submit' value='立即搜索',name='submit'></form>{$setuHtml}</h3></div></div></div></body><html>";
    }else{
