...
if (!$asked){
    if ($verifyR18 == true && !$over18){
        ...
        $htmlDoc = htmlHeader("license")."{$htmlLicense}<input id='submit' type='submit' value='Link Start !',name='submit'></form></h3><div class='notice'><p><b>欢迎使用Lolicon.App涩图检索工具2.0！</b></p><p>请阅读以下说明:</p><p>搜索时支持AND和OR规则，方法如下:</p><p>1.你可以使用逗号【,】来获取包含特定多个tag的涩图(最多3个)，如【可莉,白丝】</p><p>2.你可以使用分割符【|】来获取包含任意一个tag的涩图(最多20个)，如【黑丝|白丝|萝莉】</p>3.你可以两者混用，如【可莉,黑丝|白丝】</p><p>4.如果你只搜索一个关键词，网站将会在标题/Tag/画师中进行模糊搜索</p><p>5.网站也支持搜索特定画师的涩图，你可以使用类似如下语句【UID:画师UID】进行搜索(注意这种搜索方式不能包含其他关键词)</p><p>6.{$pidInfo}</p><p>7.规则语句会被自动修正，所以您同样可以使用中文标点，如【可莉，兽耳｜泳装】</p><p>8.网站支持一次性获取最多{$maxNumber}张图片，请冲的稍微节制一些(doge)</p><p>9.网站支持添加以下GET参数以小幅度更改网站的显示效果：<ul><li>Proxy：Pixiv反代地址</li><li>PixivAPI：Pixiv原图下载接口</li><li>Size：图片大小，你可以使用值\"original\"以获取原图</li><li>ShowTags：显示tag数量，你可以使用值\"0\"以获取全部Tag</li></ul>{$help}</p><p>最近1小时API可用性：</p><iframe width='100%' src='https://api.lolicon.app/setu/availability.html' height='150' scrolling='no'></iframe><p>注：炸了说明有憨P在爬虫CC本站使用的API</p><p>目前可以透露的情报：</p><iframe width='100%' src='https://charts.mongodb.com/charts-setu-api-qxdzw/embed/charts?id=f973f61b-912b-4dd6-b00a-f9c52db92918&amp;attribution=false'></iframe><iframe width='100%' src='https://charts.mongodb.com/charts-setu-api-qxdzw/embed/charts?id=b23a40a5-8c50-48ae-a950-95c81f9b013c&amp;attribution=false'></iframe>{$R18Info}<footer id='footer'><hr><p>使用API：<a target='_blank' href='https://api.lolicon.app' style='color:#1E90FF'>神代绮凛的Lolicon.App随机涩图</a></p><p>在此感谢神代绮凛大佬您对人类进步做出的贡献，涩图即是人类进步的原动力！</p><p class='copyright'>2022 &copy; Powered by 0803QwQ</p></footer></div></div></div><script>document.getElementById(\"here\").innerHTML = window.location.href;</script>".$htmlEnd;
    };
}else{
